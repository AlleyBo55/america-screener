...
export function LiveVisitorWidget() {
    const { count, connected } = useFirebasePresence();
    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 mb-2 opacity-80 uppercase text-[10px] font-bold tracking-widest">
                <span className={`w-2 h-2 rounded-full ${connected ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500'} animate-pulse`} />
                Live Now
            </div>
            ...
            {/* Tooltip hint */}
            <div className="absolute -bottom-8 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] text-white/40 font-mono text-center">
                Real-time
            </div>
        </motion.div>
    );
